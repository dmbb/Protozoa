...
EncodedImageCallback::Result RtpVideoSender::OnEncodedImage(
  ...
  #if MEASURE_RTP_VIDEO_SENDER_TIME_HOOK
    auto time_before_encoding = std::chrono::high_resolution_clock::now();
  #endif

  #if PROTOZOA_RECORD_FRAME_HOOK_SENDER
    ...
    /********************************* END OF FRAME PARSING ********************************/
      ...
      #if ENCODE_AT_PROTOZOA_SEND_VIDEO_HOOK
        
        uint8_t* stream_ = encoded_image.buffer();
        ...
        encodeDataIntoFrame(encoded_image.mutable_data(), encoded_image.size(), dct_partition_offset);
      #endif

      #if ENCODE_AT_PROTOZOA_SEND_VIDEO_FULL_FRAME_REPLACEMENT_HOOK
        encodeDataIntoFrame(encoded_image.mutable_data(), encoded_image.size(), first_part_offset);
      #endif
  ...
  #if PROTOZOA_SEND_VIDEO_HOOK
  //auto time_before_quit = std::chrono::high_resolution_clock::now();
  //std::chrono::duration<double, std::milli> duration2 = time_before_quit - time_before_parse;
  //printf("Time to send frame: %f\n", duration2.count());
    #if READ_AT_PROTOZOA_SEND_VIDEO_HOOK
    readFromPipe();
    #endif
  #endif
  ...
  #if MEASURE_RTP_VIDEO_SENDER_TIME_HOOK
    auto time_after_delivery = std::chrono::high_resolution_clock::now();
    std::chrono::duration<double, std::milli> duration = time_after_delivery - time_before_encoding;
    printf("[rtp_video_sender.cc] Time to send frame: %f\n", duration.count());
  #endif
